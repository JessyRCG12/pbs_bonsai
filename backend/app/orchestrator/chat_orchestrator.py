...
class SpeciesScorer:
    ...
    def score_species(self, profile: BuyerProfile, top_k: int = 3) -> List[Dict]:
        ...
        for r in rows:
            ...
            # Light match (simplified: user light vs assumed medium species need)
...
def extract_quick_profile_from_text(message: str) -> Dict:
    msg = message.lower()
    exp = "beginner" if "beginner" in msg else ("advanced" if "advanced" in msg else ("intermediate" if "intermediate" in msg else None))
    env = "indoor" if "indoor" in msg else ("outdoor" if "outdoor" in msg else None)
    light = "high" if "high light" in msg or "full sun" in msg else ("low" if "low light" in msg or "shade" in msg else None)
    m = re.search(r"(\d+)\s?(?:eur|€)?", msg)
    budget = float(m.group(1)) if m else None
    species = None
    for s in ["ficus","ulmus","zelkova","ligustrum","juniperus","podocarpus","sageretia","carmona","serissa","portulacaria","crassula","cotoneaster","pyracantha","acer palmatum","acer"]:
        if s in msg:
            species = "Acer palmatum" if "acer" in s else s.title()
            break
    profile = {}
    if exp: profile["experience_level"] = exp
    if env: profile["preferred_environment"] = env
    if light: profile["available_light"] = light
    if budget is not None: profile["budget_eur"] = budget
    if species: profile["species_hint"] = species
    return profile
        ...
        with mlflow.start_run(run_name="chat-session"):
            ...
            if mode == "direct":
                ...
                result = {
                    ...
                    "species_candidates": [{"species": species_hint, "species_score": 1.0, "rationale": "user provided"}] if species_hint else [],
                ...
                with mlflow.start_run(run_name="llm-intake", nested=True):
                    ...
                    mlflow.log_param("llm_provider", "stub")
                    mlflow.log_param("prompt_version", "v0")
            ...
            (out_dir / "buyer_profile.json").write_text(json.dumps(result["buyer_profile"], indent=2), encoding="utf-8")
            (out_dir / "species_candidates.json").write_text(json.dumps(result["species_candidates"], indent=2), encoding="utf-8")
            (out_dir / "recommendations.json").write_text(json.dumps(result["items"], indent=2), encoding="utf-8")
...
if __name__ == "__main__":
    ...
    demo = orch.handle_message("I am a beginner with 120 EUR, I prefer indoor and I have medium light. I like Ficus.", k=5)
